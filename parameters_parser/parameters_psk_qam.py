...
class ParametersPskQam(ParametersDigitalModem):
    ...
    def read_params(self, file_name: str) -> None:
        ...
        self._pulse_width = cfg.getfloat("pulse_width", fallback=1.0)
    ...
    def _check_params(self) -> None:
        ...
        if self.filter_type == "RECTANGULAR":
            if self._pulse_width <= 0 or self._pulse_width > 1:
                raise ValueError(
                    f'{msg_header}pulse width ({self._pulse_width}) must be in the interval (0, 1]')
            samples_in_rect = self._pulse_width * self.oversampling_factor
            if not float(int(samples_in_rect)) == samples_in_rect:
                raise ValueError(
                    f'{msg_header}pulse width * oversampling factor must be an integer')

            self.bandwidth = self.symbol_rate / self._pulse_width
