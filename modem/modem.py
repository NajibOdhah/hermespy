...
class Modem(Generic[P]):
    ...
    def __init__(self, param: P, source: BitsSource,
        ...
        self.rf_chain = RfChain(param, random_number_gen)
    ...
    def _add_frame_to_drop(self, initial_sample_num: int,
        ...
        """
            Args:
                frame(np.ndarray): Baseband complex samples of transmission frame.
        """

        initial_sample_idx = samples_delay + initial_sample_num
        end_sample_idx = initial_sample_idx + frame.shape[0]

        if end_sample_idx > tx_signal.shape[0]:
            # last frame may be larger than allocated space, because of
            # filtering
            tx_signal = np.append(
                tx_signal, np.zeros(
                    end_sample_idx - tx_signal.shape[0]))
    ...
    def receive(self, input_signal: np.ndarray,
                snr_linear_esn0: float) -> List[np.array]:
