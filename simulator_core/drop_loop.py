...
class DropLoop:
    ...
    def run_loop(self) -> Statistics:
        ...
        while not done:
            ...
            rx_signal = self._receive_signals(tx_signal)
    ...
    def _receive_signals(
            self, tx_signal: List[np.ndarray]) -> List[np.ndarray]:
        """Receives signals.

        Signal is propagated through channel, resampled, and afterwards
        detoriated with noise.
        ...
        for idx_rx, modem in enumerate(self._scenario.rx_modems):
            ...
            self._add_noise(
                modem,
                rx_signal,
                self._scenario.noise[idx_rx],
                output,
                idx_rx)
        return output

    def _add_noise(self, rx_modem: Modem, rx_signal: List[np.array], noise: Noise, output: List[np.ndarray],
                   idx_rx: int) -> None:
        ...
        Args:
            ...
            noise (Noise): Noise to add to the signal.
            output (List[np.ndarray]):
        ...
        for snr in self._statistics.get_snr_list(idx_rx):
            rx_signal_noisy = noise.add_noise(rx_signal, snr, energy)
            snr_linear_esn0 = noise.db_to_linear(snr)
            rx_signal_noisy_in_frames = rx_modem.receive(
                rx_signal_noisy, snr_linear_esn0)  # a list with `#frames` elements
