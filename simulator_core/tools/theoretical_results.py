    ...
    def get_results(self, snr_type: str,
        ...
        for modem_idx, modem in enumerate(self.param.rx_modem_params):
            ...
            if isinstance(modem.technology,
                ...
                bler = 1 - (1 - ber) ** bits_in_frame
                results[-1] = {'ber': ber, 'bler': bler,
                               'notes': 'AWGN channel'}
                ...
                bler = 1 - (1 - ser) ** modem.technology.number_data_chirps
                ...
                for frame_element in modem.technology.frame_structure:
                    ...
                    if frame_element[0] == modem.technology.link_direction:
                        bits_in_frame += (modem.technology.number_occupied_subcarriers *
                                          np.log2(modem.technology.modulation_order))

                        number_of_data_samples += frame_element[1]
                        number_cyclic_prefix_samples += cyclic_prefix_length

                    if frame_element[0] == 'CYCLIC_PREFIX':
                        cyclic_prefix_length = frame_element[1]
                    else:
                        cyclic_prefix_length = 0
