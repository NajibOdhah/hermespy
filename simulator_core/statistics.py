...
class Statistics:
    ...
    def update_tx_spectrum(self, all_tx_signals: List[np.ndarray]) -> None:
        ...
        if self.param_general.calc_spectrum_tx:
            for sampling_rate, frequency_range, periodogram, tx_signal in zip(
                ...
                if tx_signal.size < self.param_general.spectrum_fft_size:
                    tx_signal = np.concatenate((tx_signal, np.zeros(self.param_general.spectrum_fft_size -
                                                                    tx_signal.size)))
        ...
        if self.param_general.calc_stft_tx and self._stft_freq_tx[0].size == 0:
            ...
            for sampling_rate, tx_signal in zip(
                ...
                f, t, Zxx = signal.stft(tx_signal, sampling_rate,
                                        nperseg=self.param_general.spectrum_fft_size,
                                        noverlap=self.param_general.spectrum_fft_size / 2,
                                        return_onesided=False)
    ...
    def update_error_rate(
        ...
        for rx_modem_idx, received_signals in enumerate(detected_bits):
            ...
            for signal_idx, snr_idx in enumerate(snr_indices):
                ...
                self.bler[rx_modem_idx][snr_idx] = np.append(
                    self.bler[rx_modem_idx][snr_idx],
                ...
                self.bler_mean[rx_modem_idx][snr_idx] = np.mean(
                    self.bler[rx_modem_idx][snr_idx]
                )
                self.bler_lower[rx_modem_idx][snr_idx] = np.min(
                    self.bler[rx_modem_idx][snr_idx]
                )
                self.bler_upper[rx_modem_idx][snr_idx] = np.max(
                    self.bler[rx_modem_idx][snr_idx]
                )
                ...
                if self.num_drops >= self.param_general.min_num_drops:
                    ...
                    bler_lower = self.bler_lower[rx_modem_idx][snr_idx]
                    ber_upper = self.ber_upper[rx_modem_idx][snr_idx]
                    bler_upper = self.bler_upper[rx_modem_idx][snr_idx]
                    ...
                    # do the same stuff for BLER
                    bler_rx = self.bler[rx_modem_idx][snr_idx]

                    if self.num_drops > 1:
                        bler_stats = stats.bayes_mvs(
                            bler_rx, alpha=self.param_general.confidence_level
                        )

                        if not np.isnan(bler_stats[0][1][0]):
                            bler_lower = bler_stats[0][1][0]
                            if bler_lower < 0:
                                bler_lower = 0
                            bler_upper = bler_stats[0][1][1]
                    else:
                        bler_lower = self.bler[rx_modem_idx][snr_idx]
                        bler_upper = self.bler[rx_modem_idx][snr_idx]

                    self.ber_lower[rx_modem_idx][snr_idx] = ber_lower
                    self.bler_lower[rx_modem_idx][snr_idx] = bler_lower
                    self.ber_upper[rx_modem_idx][snr_idx] = ber_upper
                    self.bler_upper[rx_modem_idx][snr_idx] = bler_upper
            ...
            if (
                ...
                elif self.param_general.confidence_metric == "BLER":
                    confidence_margin = (
                        self.bler_upper[rx_modem_idx] -
                        self.bler_lower[rx_modem_idx]
                    ) / self.bler_mean[rx_modem_idx]
            ...
            if self.param_general.verbose:
                ...
                for snr, idx in zip(self.get_snr_list(
                    ...
                    print(
                        "\tSNR = {:f} dB, BER = {:f}, BLER = {:f}".format(
                            snr,
                            self.ber[rx_modem_idx][idx][-1],
                            self.bler[rx_modem_idx][idx][-1],
                        )
                    )
    ...
    def save(self, results_dir: str) -> None:
        ...
        if self.param_general.snr_type == 'EB/N0(DB)':
            snr_str = 'Eb/N0'
        elif self.param_general.snr_type == 'EB/N0(DB)':
            snr_str = 'Es/N0'
        ...
        for rx_modem_idx in range(self.param_scenario.number_of_rx_modems):
            ...
            for snr, ber, bler in zip(
                self.param_general.snr_vector, self.ber_mean[rx_modem_idx], self.bler_mean[rx_modem_idx]
            ):
                print(f"\t{snr_str} = {snr}dB\tBER = {ber:e}, \tBLER = {bler:e}")

        mat_dict = {
            "snr_type": self.param_general.snr_type,
            "snr_vector": self.param_general.snr_vector,
            "ber_mean": self.ber_mean,
            "bler_mean": self.bler_mean,
            "ber_lower": self.ber_lower,
            "ber_upper": self.ber_upper,
            "bler_lower": self.bler_lower,
            "bler_upper": self.bler_upper,
        }
        if self.param_general.calc_spectrum_tx:
        ...
        if theory is not None:
            for rx_modem_idx in range(self.param_scenario.number_of_rx_modems):
                if not theory[rx_modem_idx]:
                    ...
                    bler_theory.append(np.nan)
                    theory_notes.append("")
                else:
                    ber_theory.append(theory[rx_modem_idx]["ber"])
                    if "bler" in theory[rx_modem_idx].keys():
                        bler_theory.append(theory[rx_modem_idx]["bler"])
                    else:
                        bler_theory.append(np.nan)
                    theory_notes.append(theory[rx_modem_idx]["notes"])

            mat_dict["ber_theory"] = ber_theory
            mat_dict["bler_theory"] = bler_theory
        ...
        if self.param_general.plot:
            ...
            for rx_modem_idx in range(self.param_scenario.number_of_rx_modems):
                ...
                if bler_theory[rx_modem_idx] is not np.nan:
                    plt.plot(
                        self.param_general.snr_vector,
                        bler_theory[rx_modem_idx],
                        label="theory")

                error = np.vstack(
                    (self.bler_mean[rx_modem_idx] - self.bler_lower[rx_modem_idx],
                     self.bler_upper[rx_modem_idx] - self.bler_mean[rx_modem_idx])
                )
                plt.errorbar(
                    self.param_general.snr_vector, self.bler_mean[rx_modem_idx], error, label="simulation"
                )
                plt.yscale("log", nonposy="mask")
                plt.title("Rx" + str(rx_modem_idx))
                plt.xlabel(snr_str + "(dB)")
                ...
                filename = os.path.join(
                    results_dir, "BLER_" + str(rx_modem_idx) + ".png")
                plt.savefig(filename)
