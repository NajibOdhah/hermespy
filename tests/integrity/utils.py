...
def compare_mat_files(
    ...
    if not nan_arrays_equal(
        ...
        results_equal = False
    elif not nan_arrays_equal(simulation_mat["bler_mean"], expected_results_mat["bler_mean"]):
        results_equal = False
    elif not nan_arrays_equal(simulation_mat["ber_lower"], expected_results_mat["ber_lower"]):
        results_equal = False
    elif not nan_arrays_equal(simulation_mat["bler_lower"], expected_results_mat["bler_lower"]):
        results_equal = False
    elif not nan_arrays_equal(simulation_mat["ber_upper"], expected_results_mat["ber_upper"]):
        results_equal = False
    elif not nan_arrays_equal(simulation_mat["bler_upper"], expected_results_mat["bler_upper"]):
        results_equal = False

    for tx_idx in range(no_tx_modems):
        if not nan_arrays_equal(
                simulation_mat[f"frequency_tx_{tx_idx}"], expected_results_mat[f"frequency_tx_{tx_idx}"]):
            results_equal = False
        elif not nan_arrays_equal(simulation_mat[f"power_spectral_density_tx_{tx_idx}"], expected_results_mat[f"power_spectral_density_tx_{tx_idx}"]):
            results_equal = False
        elif not nan_arrays_equal(simulation_mat[f"stft_power_tx{tx_idx}"], expected_results_mat[f"stft_power_tx{tx_idx}"]):
            results_equal = False
