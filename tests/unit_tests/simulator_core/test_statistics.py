...
class StatisticsTest(unittest.TestCase):
    ...
    def test_update_error_rate_one_drop_two_frames_multiple_blocks(
            self) -> None:
        """Tests BLER calculation for one drop and one frame"""
        ...
        for rx_modem_idx in range(self.no_rx_modems):
            np.testing.assert_array_almost_equal(
                np.array(self.BLER_expected[rx_modem_idx]),
                np.array(self.stats.bler[rx_modem_idx]),
            )
    ...
    def test_update_tx_spectrum_welch_method_update_periodogram(
        ...
        mock_welch.assert_called_once_with(tx_signal, fs=self.stats._tx_sampling_rate[0],
                                           nperseg=self.stats.param_general.spectrum_fft_size,
                                           noverlap=self.stats.param_general.spectrum_fft_size / 2,
    ...
    def test_stft_calculation_tx(
        ...
        # check method calls
        mock_stft.assert_called_once_with(
            tx_signal, self.stats._tx_sampling_rate[0],
            nperseg=self.stats.param_general.spectrum_fft_size,
            noverlap=self.stats.param_general.spectrum_fft_size / 2,
            return_onesided=False
        )
