...
class TestDropLoop(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        mock_params_general.max_num_drops = 3
    ...
    def test_noise_addition_proper_method_call(self) -> None:
        ...
        mock_noise = Mock()
        mock_noise.add_noise.return_value = (rx_signal)

        output: List[np.ndarray] = []

        self.drop_loop._add_noise(
            mock_rx_modem, rx_signal, mock_noise, output, 0)
    ...
    def test_proper_method_calls_receive_signals(self) -> None:
        ...
        self.drop_loop._add_noise = Mock()  # type: ignore
        self.drop_loop._add_noise.return_value = None

        self.drop_loop._scenario.noise = [Mock()] * no_rx_modems

        # call actual function
        _ = self.drop_loop._receive_signals(tx_signal)
        ...
        self.assertEqual(self.drop_loop._add_noise.call_count, no_rx_modems)
