...
class MultipathFadingChannel(Channel):
    ...
    def __init__(self, parameters: ParametersChannel, number_tx_antennas: int,
                 number_rx_antennas: int, random_number_gen: np.random.RandomState,
                 sampling_rate: float, doppler_frequency: float = None,
                 tx_cov_matrix: np.ndarray = None, rx_cov_matrix: np.ndarray = None) -> None:
        super().__init__(parameters, number_tx_antennas,
                         number_rx_antennas, random_number_gen, sampling_rate)
        ...
        if rx_cov_matrix is None:
            self.postcoding_matrix = np.array([])
        else:
            if not np.array_equal(rx_cov_matrix, np.transpose(rx_cov_matrix)):
                raise ValueError('rx covariance matrix must be Hermitian')
            if np.any(np.eigvals(rx_cov_matrix) <= 0):
                raise ValueError(
                    'rx covariance matrix must be positive definite')

            self.postcoding_matrix = np.sqrt(rx_cov_matrix)

        # precoding matrix for transmit antenna correlation
        if tx_cov_matrix is None:
            self.precoding_matrix = np.array([])
        else:
            if not np.array_equal(tx_cov_matrix, np.transpose(tx_cov_matrix)):
                raise ValueError('tx covariance matrix must be Hermitian')
            if np.any(np.eigvals(tx_cov_matrix) <= 0):
                raise ValueError(
                    'tx covariance matrix must be positive definite')

            self.precoding_matrix = np.sqrt(tx_cov_matrix)
        ...
        self._number_of_los_components = [
            self.number_of_paths, number_rx_antennas, number_tx_antennas]
    ...
    def propagate(self, tx_signal: np.ndarray) -> np.ndarray:
        ...
        Args:
            tx_signal (np.ndarray): Input signal to channel.
        ...
        if self.precoding_matrix.size > 0 or self.postcoding_matrix.size > 0:
            for instant in range(timestamps.size):
                for delay in delay_in_samples_vec:
                    ...
                    mimo_channel = np.squeeze(mimo_channel, axis=(0, 3))
                    mimo_channel = (
    ...
    def _get_path_responses(
        ...
        for path in range(self.number_of_paths):
            ...
            # delay profile
            los_component = np.exp(1j * (2 * np.pi * self.doppler_frequency *
                                         np.cos(
