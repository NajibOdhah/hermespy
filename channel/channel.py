...
class Channel:
    ...
    def __init__(
        ...
        self.ideal_mimo_response = np.ndarray([])
        if self.number_rx_antennas == self.number_tx_antennas:
            self.ideal_mimo_response = np.eye(self.number_rx_antennas)
        elif self.number_rx_antennas > self.number_tx_antennas:
            self.ideal_mimo_response = np.concatenate(
                (
                    np.eye(self.number_tx_antennas),
                    np.zeros(
                        (
    ...
    def propagate(self, tx_signal: np.ndarray) -> np.ndarray:
        ...
        For the ideal channel in the base class, the MIMO channel is modeled as an
        identity matrix, i.e., let `N` be the minimum number of either receive or
        transmit antennas, then the `N` first outputs will be the same as the `N`
        first inputs.
